...
- **Canonical project root is `/home/sergi/Documentos/advanced-rag/`.** Hermes runs elsewhere; `~/.hermes/plugins/advanced-rag/` does not exist on this machine and must not be created here.
...
- `advanced_rag/__init__.py::register(ctx)` — wires everything into `ctx.register_*`.
- `advanced_rag/adapters.py` — closures that reshape pure handlers to whatever signature Hermes wants. Lazy imports inside each closure so a missing pure module fails loud during dev rather than at registration.
...
- **Data dir precedence (config.py).** Explicit `Store(data_dir=…)` arg > `HERMES_RAG_DATA_DIR` env > default `~/.hermes/plugins/advanced-rag/data/`. Don't add a fourth path.
...
python -c "from advanced_rag import register"  # smoke-imports the plugin
...
- `advanced_rag/requirements.txt` is a copy of the repo-root `requirements.txt`; the duplication is intentional so a single `rsync -av advanced_rag/ …` carries deps to runtime in one shot. If you change one, change both.
